...
def compress(array, err_threshold, extra_channels = True, verbose = False, method='None', mode = 'None', convs = 4, hyp = 1):
    ...
    if method == 'None':
        ...
        error_neg = error_neg.astype(np.int64)
        ...
        huffman = True
            ...
            #mask1 = map(str, mask)
            #mask = ','.join(mask)
            #b = getHuffmanCode(mask1)
            #a = decode_huffman(a, b.keys(), b.values()) 
            #print(a)
    ...
    return out, latent, error_m, error_e  # error, mask, decompressed, compressed_data, error_q, error_quan
