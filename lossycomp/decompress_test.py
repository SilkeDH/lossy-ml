...
from lossycomp.huffman import decode_huffman
import fpzip
...
def decompress(compressed_data, extra_channels = True, verbose = False,  method='None', mode = 'None', convs = 4, hyp = 1, dec_mask='bz2', enc_lat = 'bz2'):
    reset_random_seeds()
    ...
    if method == 'None':
        ...
        if enc_lat == 'bz2':
            compressed_data =  bz2.decompress(compressed_data)
            compressed_data = np.frombuffer(compressed_data, dtype=np.float64).reshape(comp_data_shape)
        elif enc_lat == 'fpzip':
        ...
        if dec_mask == 'bz2':
            ...
            mask = decode_huffman(comp_mask[0], comp_mask[1].keys(), comp_mask[1].values()) 
            mask = np.array(list(mask), dtype = np.byte)
