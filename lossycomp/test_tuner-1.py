...
def main():
    ...
    tuner = RandomSearch(
        ...
        max_trials=2,
        #max_epochs = 70,
        directory='/home/kit/scc/nq1231/Thesis/results/5_channel_hyperOpt_resv1_400k',
        distribution_strategy=tf.distribute.MirroredStrategy(),
        project_name='AE_model')

    print("Space tuner summary", flush=True)
    ...
    dg_train = DataGenerator(train, 100, leads, batch_size=10, load=True, mean= mean, std=std, coords = True, standardize = True) 
    dg_test = DataGenerator(test, 10, leads, batch_size=10, load=True, mean= mean, std=std, coords = True, standardize = True)
    
    print("Starting with search..." flush=True)
    tuner.search(dg_train, epochs=2,  validation_data=dg_test, validation_freq = 5)
